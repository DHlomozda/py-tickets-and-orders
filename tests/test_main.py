...
def movies_data(genres_data, actors_data):
    ...
    matrix2 = Movie.objects.create(
        title="Matrix 2", description="Matrix 2 movie"
    )
...
def test_user_service_create_user():
    ...
    assert (
        get_user_model().objects.get(username="User1").password
        != "Password1234"
    ), "Password should be encrypted"
    assert (
        get_user_model().objects.get(username="User2").password
        != "Password5678"
    ), "Password should be encrypted"
...
def test_user_service_update_user_with_password(users_data):
    ...
    assert (
        get_user_model().objects.get(id=1).check_password("new_password1234")
    )
...
def test_order_service_get_orders_with_user(orders_data):
    assert list(
        get_orders(username="user1").values_list("user__username")
    ) == [
        ("user1",),
        ("user1",),
    ]
...
def create_order_data():
    ...
    cinema_hall = CinemaHall.objects.create(
        name="Blue", rows=14, seats_in_row=12
    )
    MovieSession.objects.create(
